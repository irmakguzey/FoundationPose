...
class PairH5Dataset(torch.utils.data.Dataset):
            ...
            if os.path.exists(key_file):
                ...
                with h5py.File(h5_file, "r", libver="latest") as hf:
                    for k in hf:
                        ...
                        if (
                            max_num_key is not None
                            and len(self.object_keys) >= max_num_key
                        ):
            ...
            with h5py.File(h5_file, "r", libver="latest") as hf:
                ...
                for k_perturb in group:
                    ...
                    if "crop_ratio" in group[k_perturb]:
                        self.cfg["crop_ratio"] = float(
                            group[k_perturb]["crop_ratio"][()]
                        )
    ...
    def transform_depth_to_xyzmap(self, batch: BatchPoseData, H_ori, W_ori, bound=1):
        ...
        if batch.xyz_mapAs is None:
            ...
            batch.xyz_mapAs = depth2xyzmap_batch(
                depthAs_ori[:, 0], batch.Ks, zfar=np.inf
            ).permute(
                0, 3, 1, 2
            )  # (B,3,H,W)
        ...
        if batch.xyz_mapBs is None:
            ...
            batch.xyz_mapBs = depth2xyzmap_batch(
                depthBs_ori[:, 0], batch.Ks, zfar=np.inf
            ).permute(
                0, 3, 1, 2
            )  # (B,3,H,W)
        ...
        if mode != "test":
            with h5py.File(h5_file, "r", libver="latest") as hf:
                ...
                for key_perturb in group:
                    ...
                    self.cfg["n_view"] = min(
                        self.cfg["n_view"], depthA.shape[1] // depthB.shape[1]
                    )
                    ...
                    self.rot_normalizer = (
                        group[key_perturb]["rot_normalizer"][()] / 180.0 * np.pi
                    )
