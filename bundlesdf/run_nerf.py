...
def run_neural_object_field(
    ...
    cam_in_obs,
    debug=0,
    save_dir="/home/bowen/debug/foundationpose_bundlesdf",
):
...
def run_one_ob(base_dir, cfg, use_refined_mask=False):
    ...
    for i, color_file in enumerate(color_files):
        ...
        cam_in_ob = np.loadtxt(
            color_file.replace("rgb", "cam_in_ob").replace(".png", ".txt")
        ).reshape(4, 4)
        rgbs.append(rgb)
        depths.append(depth)
        masks.append(mask)
    ...
    mesh = run_neural_object_field(
        cfg, K, rgbs, depths, masks, cam_in_obs, save_dir=save_dir, debug=0
    )
    return mesh
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ref_view_dir",
        type=str,
        default=f"/mnt/9a72c439-d0a7-45e8-8d20-d7a235d02763/DATASET/YCB_Video/bowen_addon/ref_views_16",
    )
    parser.add_argument(
        "--dataset", type=str, default=f"ycbv", help="one of [ycbv/linemod]"
    )
